...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('home', '0012_rentalanalysis_salesprices'),
    ]
    ...
    operations = [
        migrations.RunSQL(
            ...
            LEFT JOIN `agenthouse` ON (`house`.`house_id` = `agenthouse`.`house_id`)
            LEFT JOIN `agent` ON (`agenthouse`.`agent_id` = `agent`.`agent_id`) 
            LEFT JOIN `agencyhouse` ON (`house`.`house_id` = `agencyhouse`.`house_id`)
            LEFT JOIN `agency` ON (`agencyhouse`.`agency_id` = `agencyhouse`.`agency_id`);
            '''
        ),
    ]
